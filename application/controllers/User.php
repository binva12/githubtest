...
class User extends CI_Controller
{
	...
	public function form_submit_method(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if($this->user_model->insert_into_user_table($data_array)){
				...
				$this->session->set_flashdata('error', 'Failed to create the user');
				
				redirect("helpers/form");
			}
			
			// $get = $this->db->query("SELECT * FROM tbl_user WHERE id=? ", [$input["id"]])->row_array();
			// print_r($get);
		}
	}
	...
	function list_all_users() {
		$list_user = $this->user_model->get_all_user();
