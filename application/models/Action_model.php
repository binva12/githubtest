<?php
	...
	class Action_model extends CI_Model{
		...
		public function select_all_data(){
			...
			// it will select all data form table
			/* $this->db->select("*");
		...
		public function delete_specific_user(){
			...
			//delte from tbl_users where id = 4
		...
		public function get_users_messages(){

			// join tbl_users =>id, match user_id tbl_messages
			...
			$this->db->join("tbl_messages as message", "user.id = message.user_id", "");
			$query = $this->db->get();
			// select user.*, messages.message from tbl_user as user INNER JOIN tbl_messages as message ON user.id = message.user_id
			return $result = $query->result(); 
		}
	}
